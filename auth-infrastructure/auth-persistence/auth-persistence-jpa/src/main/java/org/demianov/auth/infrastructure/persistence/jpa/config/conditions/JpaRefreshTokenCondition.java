...
/**
 * Condition to check if the JPA refresh token repository is enabled.
 */
public class JpaRefreshTokenCondition implements Condition {

    /**
     * Checks if the JPA refresh token repository is enabled.
     * @param context the condition context
     * @param metadata the metadata of the {@code AnnotationMetadata class}
     * or {@code MethodMetadata method} being checked
     * @return {@code true} if the JPA refresh token repository
     * is enabled,
     */
    @Override
    public boolean matches(
        ...
        String type = context.getEnvironment()
                .getProperty("auth.repository.refresh-token.type");
